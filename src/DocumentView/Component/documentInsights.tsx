...
interface Props {
    ...
    saveDataPoints: (dataPoints: string[], state: boolean, tempParaIndex: number, type: string) => void;
...
export default class DocumentInsights extends Component<Props, State> {
    ...
    componentWillReceiveProps(nextProps: Props) {
        ...
        if (!isNullOrUndefined(this.props.location.state) && this.props.location.state === nextProps.location.state) {
    ...
    onClickCreateTask = (name: string, value: string, contractName: string) => {
        let { setCurrentTask, setTaskPage, getAllTasksData, fileId, setTaskOrigin } = this.props;
    ...
    render() {
        let { insightsData, sentenceLoader, setCurrentTask, clauseModeStatus } = this.props;
                                            ...
                                            <div className="row">
                                                <div className="col-md-12">
                                                    <div className="row">
                                                        ...
                                                        >
                                                            <img alt="Create Task" src="/static_images/task-icn.svg" />
                                                        </div>
                                                ...
                                                >
                                                    <div className="col-md-12">
                                                        {data.children.map((child, j) => (
                                                            <div key={j}>
                                                                {this.childrenRender(child, j, false, [], -1, '')}
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
    ...
    childrenRender(
        ...
        let { fileId, editDataPoint, saveInsightToDelete, durationList, currencyList } = this.props;
            ...
            return (
                <>
                    <EditFeature
                        ...
                        savedInsight={!isNullOrUndefined(child.childValue) ? child.childValue : []}
                    ...
                    />
                    {child.childValue.length > 0 && alternateNewBIs.indexOf(child.childLabel) > -1 && (
                            ...
                            style={{
                                ...
                                marginBottom:
                                    child.childLabel === 'Consent Regulatory'
                                        ? '1px'
                                        : child.childLabel === ('Consent Authority' || 'Payment Obligations Nature')
                                        ? '10px'
                                        : '',
                            }}
                        >
                                ...
                                <div className="row">
                                        ...
                                        style={{
                                            fontWeight:
                                                ...
                                                (child.childAlias === 'Present' ||
                                                    ...
                                                    disputeResolutionModeBi.indexOf(child.childAlias) > -1)
                                                    ? 600
                                                    : 400,
                                        }}
                                    >
                                    ...
                                    >
                                        {hideBIValue.indexOf(child.childLabel) < 0 &&
                                            clickedSubMenu === child.childLabel && (
                                                <span className="bi-count">{child.childValue.length}</span>
                                            )}
                                    </div>
                                                ...
                                                >
                                                    {alternateNewBIs.indexOf(child.childLabel) > -1 ? (
                                                        ...
                                                        <DarkTooltip title={'Edit'} placement="right-end">
                                                            <img src="/static_images/new-edit-icon.svg" alt="edit" />
                                                        </DarkTooltip>
                                                    )}
                                                </span>
                                            )}
                            ...
                            {separateUnclickableBI.indexOf(child.childLabel) > -1 ? (
                                <div className="col-md-12">
                                    <div className="row" style={{ paddingBottom: '0.5rem' }}>
                                        ...
                                        <div className="col-md-1 p-0 mt-2 mb-2">{/* do nothing */}</div>
                                        <div className="col-md-1 p-0 mt-2 mb-2">{/* do nothing */}</div>
                                        <div className="col-md-1" style={{ margin: 'auto' }}>
                                            {child.childValue.length > 0 &&
                                            (!isNullOrUndefined(child.childValue[0]) ||
    ...
    setClickedItem(clickedItem: string, type: string, unselectChild: boolean) {
        let { highlightedYellow, highlightedLilac, clickedMenu, clickedSubMenu, clickedInstanceIndex } = this.state;
        if (clickedItem === 'clause') {
            this.setState({ clickedSubMenu: '', clickedInstanceIndex: 0 });
            clickedMenu === type ? this.setState({ clickedMenu: '' }) : this.setState({ clickedMenu: type });
    ...
    scrollToChildDataPoint = (
        ...
        if (children.length > 0) {
            ...
            multipleInstances =
                type === 'Termination at Convenience' || disputeResolutionModeBi.indexOf(type) > -1
                    ? detectMultipleInstances(children)
                    : children;
            let paraHeader = document.getElementById(multipleInstances[index].dataPoints[index]);
            !isNullOrUndefined(paraHeader) && paraHeader.scrollIntoView({ block: 'center' });
        ...
        this.props.saveDataPoints(
            aggregateDataArray.length === 0 ? savedParentDataPoints : aggregateDataArray,
            true,
