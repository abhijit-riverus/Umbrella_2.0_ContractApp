...
import { CurrencyType, editedPhrases, LinkPhraseRequest, phraseInfo, tableInfo } from '../../../../State/documentState';
...
export default class LinkAmountPhrases extends Component<Props, State> {
    ...
    getPhraseEdit() {
        let {  phraseInAddMode, currencyList } = this.props;
        let {currencyValue, selectedOption, hideOptions} = this.state;
        return (
            <div className="row">
                ...
                <div className="col-md-8" style={{ color: '#4D4D4D', fontSize: '14px' }}>
                        <div className="row">
                            <div className="col-md-4 py-2">
                                <input type="text" className="currency-input" value={currencyValue === -1 ? '' : currencyValue} placeholder="Enter no." onChange={(e) => this.setCurrencyValue(e)} style={{ width: '100%' }} />
                            </div>
                            <div className="col-md-6 py-2">
                                <span className="filter-modal-input" style={{ background: 'white', border: '1px solid #DDDDD', lineHeight: '24px' }}  onClick={() => this.setState({ hideOptions: !this.state.hideOptions})} >
                                    <input type="text" className="modal-input" placeholder="Select currency" style={{ width: '100%', border: 'none', outline: 'none' }} value={selectedOption.currencyName } readOnly  />
    ...
    setCurrencyValue(event: any){
        ...
        this.setState({ currencyValue: currency });
    }
    ...
    getSaveStatus(){
        ...
        let {currencyValue, selectedOption} = this.state;
        let saveStatus: boolean = false;
        if(phraseInAddMode !== null && phraseInAddMode.paraId !== null && phraseInAddMode.paraId !== -1 && currencyValue !== -1 && selectedOption.currencyTypeId !== -1){
            saveStatus = true;
        }
        return saveStatus;
    }
    ...
    onSave() {
        let { phraseInAddMode } = this.props;
        let {selectedOption, currencyValue} = this.state;
