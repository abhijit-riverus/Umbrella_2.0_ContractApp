...
export function mapDispatchToProps(dispatch: any, ownProps: any) {
  return {
    uploadFiles: (fileChunk: any, fileInfo: FileInfo[]) =>
      dispatch(UploadActionGenerator.fileUpload(fileChunk, fileInfo)),
    ...
    deleteFile: (fileId: string) =>
      dispatch(HistoryActionGenerator.deleteFile(fileId)),
    storeUploadValidity: (
      uploadValidityObject: UploadValidityObject,
      totalFiles: File[]
    ) =>
      dispatch(
        UploadActionGenerator.storeUploadValidity(
          uploadValidityObject,
          totalFiles
        )
      ),
    checkDuplicate: (fileNameArray: string[], file: File[]) =>
      dispatch(UploadActionGenerator.checkDuplicate(fileNameArray, file)),
