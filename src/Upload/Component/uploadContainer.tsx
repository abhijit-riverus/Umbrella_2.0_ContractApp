...
interface Props {
    ...
    storeUploadValidity: (uploadValidityObject: UploadValidityObject, totalFiles: File[]) => void;
    totalFiles: File[];
...
export default class UploadContainer extends Component<Props, State> {
    ...
    switchRender() {
        let { fileInfo, uploadFiles, userName, uploadLoader, checkDuplicate, history, toBeUploaded, storeUploadValidity, saveDeleteDetails } = this.props;
        if (fileInfo.length > 0) {
                            ...
                            <Scrollable maxHeight={370} minHeight={'50vh'}>
                                {fileInfo.map((file, i) =>
                                    <React.Fragment key={i}>
                                        <FileListComponent file={file} history={history} from={"addfiles"} saveDeleteDetails={saveDeleteDetails} />
                                    </React.Fragment>
                                )}
                            </Scrollable>
                        </div>
            ...
            return (
                <div onClick={() => this.setState({ invokeInput: false })}>
                    <UploadModule toBeUploaded={toBeUploaded} checkDuplicate={checkDuplicate} fileList={fileInfo} invokeInput={false}
                        userName={userName} uploadFiles={uploadFiles} storeUploadValidity={storeUploadValidity} />
                </div>
            )
        }
    }
    ...
    render() {
        let { fileInfo, uploadFiles, userName, checkDuplicate, history, toBeUploaded, storeUploadValidity } = this.props;
        let { selectedFilter } = this.state;
        return (
            <div className="row">
                        ...
                        <div className="col-md-10 mt-3 ml-5">
                            <div className="row">
                                <div className="col-md-10">
                                    <QuickLook fileInfo={fileInfo} selectFilter={this.selectFilter} selectedFilter={selectedFilter} />
                                </div>
                                {fileInfo.length > 0 && <div className="col-md-1 p-0" onClick={() => this.setState({ invokeInput: true })}>
                                    <UploadModule toBeUploaded={toBeUploaded} checkDuplicate={checkDuplicate} fileList={fileInfo} invokeInput={true}
                                        userName={userName} uploadFiles={uploadFiles} storeUploadValidity={storeUploadValidity} />
                                </div>}
                            </div>
                            <div className="row">
                                <div className="col-md-9"></div>
                                <div className="col-md-3 message-link" onClick={() => window.location.href = '/uploads'}>
                                    {fileInfo.length > 0 && "View all processed files"}
                                </div>
                                <button style={{ display: 'none' }} type="button" data-toggle="modal" data-target="#fileStatusModal" id="fileStatusButton"></button>
                            </div>
                            {this.switchRender()}
                        </div>
