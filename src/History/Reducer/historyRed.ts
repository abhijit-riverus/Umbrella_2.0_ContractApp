import { GETUSERUPLOADS_SUCCESS, CHANGESTATUS_SUCCESS, HistoryActions, GETUSERUPLOADS, DELETEFILE_SUCCESS, DELETEFILE_FAILURE, SAVEDELETEDETAILS } from "../Actions/def";
...
export default function historyReducer(state: HistoryState = defaultHistoryState(), action: HistoryActions): HistoryState {
    switch (action.type) {
        case GETUSERUPLOADS: {
            return { ...state, historyLoader: true };
        }
        case GETUSERUPLOADS_SUCCESS: {
            return { ...state, userUploads: defineProcessStatus(action.payload.userUploads.slice(0)), historyLoader: false };
        }
        case CHANGESTATUS_SUCCESS: {
            return { ...state, userUploads: changeProcessStatus(state.userUploads.slice(0), action.payload.statusPoints) };
        }
        ...
        case SAVEDELETEDETAILS: {
            return { ...state, documentName: action.payload.documentName, uniqueFileId: action.payload.uniqueFileId }
        }
