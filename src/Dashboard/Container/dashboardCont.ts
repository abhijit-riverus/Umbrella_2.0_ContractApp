...
export function mapDispatchToProps(dispatch: any, ownProps: any) {
    return {
        ...
        getJurisdiction: (fileIds: number[]) => dispatch(DashboardActionGenerator.getJurisdiction(fileIds)),
        getUploadedBy: (fileIds: number[]) => dispatch(DashboardActionGenerator.getUploadedBy(fileIds)),
        getContractType: (fileIds: number[]) => dispatch(DashboardActionGenerator.getContractType(fileIds)),
        getTableData: (fileIds: number[]) => dispatch(DashboardActionGenerator.getTableData(fileIds)),
        getDashboardFileId: () => dispatch(DashboardActionGenerator.getDashboardFileId()),
        ...
        applyDashboardFilter: (fileIds: number[], filterStructure: DashboardFilterStructure[]) => dispatch(DashboardActionGenerator.applyDashboardFilter(fileIds, filterStructure)),
        saveDashboardFilters: (savedFilters: string[]) => dispatch(DashboardActionGenerator.savedDashboardFilters(savedFilters))
    }
}
