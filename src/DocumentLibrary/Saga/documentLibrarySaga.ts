	...
	try {
		...
		switch (response.status) {
			case 200: {
				...
				yield put(
					DocumentLibraryGenerator.getLibraryFileIdSuccess(parsed)
				);
				if (parsed !== null) {
					yield put(
						DocumentLibraryGenerator.getDocumentHierarchy(
							sort,
							order,
							parsed
						)
		...
		switch (response.status) {
			case 200: {
				let parsed = response.data
					.queryResult as DocumentHierarchyData[];
				yield put(
					DocumentLibraryGenerator.getDocumentHierarchySuccess(parsed)
				);
				break;
			}
			default: {
				yield put(
					DocumentLibraryGenerator.getDocumentHierarchyFailure()
				);
			}
		}
	} catch (e) {
	...
	try {
		...
		switch (response.status) {
			case 200: {
				...
				yield put(
					DocumentLibraryGenerator.applyLibraryTagFiltersSuccess(
						parsed
					)
				);
				break;
			...
			default: {
				yield put(
					DocumentLibraryGenerator.applyLibraryTagFiltersFailure()
				);
			}
		}
	} catch (e) {
...
function* addOrEditTags(action: EditTags) {
	...
	try {
		let response = yield call(
			DocumentLibraryAPI.addOrEditTags,
			url,
			payload
		);
		switch (response.status) {
	...
	try {
		...
		switch (response.status) {
			case 200: {
				...
				yield put(
					DocumentLibraryGenerator.getParentFileListSuccess(parsed)
				);
				break;
			}
			default: {
		...
		switch (response.status) {
			case 200: {
				let parsed = response.data.queryResult
					.data as ChildrenFileInfo[];
				yield put(
					DocumentLibraryGenerator.getChildrenFileListSuccess(parsed)
				);
				break;
			}
			default: {
				yield put(
					DocumentLibraryGenerator.getChildrenFileListFailure()
				);
			}
		}
	} catch (e) {
	...
	try {
		...
		switch (response.status) {
			case 200: {
				...
				yield put(
					DocumentLibraryGenerator.getGeneralFilterDLSuccess(parsed)
				);
				break;
			}
			default: {
	...
	try {
		...
		switch (response.status) {
			case 200: {
				let parsed = response.data.queryResult.fileIds as number[];
				yield put(
					DocumentLibraryGenerator.applyGeneralFilterDLSuccess(parsed)
				);
				yield put(
					DocumentLibraryGenerator.getDocumentHierarchy(
						sort,
			...
			default: {
				yield put(
					DocumentLibraryGenerator.applyGeneralFilterDLFailure()
				);
				break;
			}
		}
