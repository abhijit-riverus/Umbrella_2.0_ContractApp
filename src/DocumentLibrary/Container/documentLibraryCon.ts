...
import { BasicFileInfo, LibraryTagFilterStructure, TagInfo } from "../State/documentLibraryState";
...
export function mapDispatchToProps(dispatch: any, ownProps: any) {
    return {
        ...
        getLibraryFileId: (sort: string, order: string) => dispatch(DocumentLibraryGenerator.getLibraryFileId(sort,order)),
        getHierarchy: (sort: string, order: string, fileIds: number[]) => dispatch(DocumentLibraryGenerator.getDocumentHierarchy(sort, order, fileIds)),
        saveMultipleSelectedFiles: (savedMultipleSelectedFiles: BasicFileInfo[]) => dispatch(DocumentLibraryGenerator.saveMultipleSelectedFiles(savedMultipleSelectedFiles)),
        saveCollapsedFileIds: (savedCollapsedFileIds: number[]) => dispatch(DocumentLibraryGenerator.saveCollapsedFileIds(savedCollapsedFileIds)),
        applyLibraryTagFilters: (appliedLibraryTagFilters: LibraryTagFilterStructure[], initialFileIds: number[]) => dispatch(DocumentLibraryGenerator.applyLibraryTagFilters(appliedLibraryTagFilters, initialFileIds)),
        ...
        saveSelectedFile: (selectedFile: BasicFileInfo | null) => dispatch(DocumentLibraryGenerator.saveSelectedFile(selectedFile)),
        saveSelectedLibraryAction: (selectedLibraryAction: string, isBulkAction: boolean) => dispatch(DocumentLibraryGenerator.saveSelectedLibraryAction(selectedLibraryAction, isBulkAction)),
        ...
        getGeneralFilter: (fileIds: number[]) => dispatch(DocumentLibraryGenerator.getGeneralFilterDL(fileIds)),
        applyGeneralFilter: (fileIds: number[], filterType: string, sort: string, order: string) => dispatch(DocumentLibraryGenerator.applyGeneralFilterDL(fileIds, filterType, sort, order)),
    }
}
